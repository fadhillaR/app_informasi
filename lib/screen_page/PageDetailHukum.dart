...
import 'package:app_informasi/screen_page/PageUpdateHukum.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:url_launcher/url_launcher.dart';

class PageDetailHukum extends StatelessWidget {
  final Datum? data;

  const PageDetailHukum(this.data, {super.key});

  Future<bool> _deleteHukum(String idHukum) async {
    final response = await http.post(
      Uri.parse('http://192.168.100.6/informasiDb/deleteHukum.php'),
      body: {
        "id_hukum": idHukum,
      },
    );
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ElevatedButton(
              onPressed: () {
                _deleteHukum(data?.idHukum ?? '').then((isSuccess) {
