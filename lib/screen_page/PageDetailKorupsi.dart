...
import 'package:app_informasi/screen_page/PageUpdateKorupsi.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:url_launcher/url_launcher.dart';

class PageDetailKorupsi extends StatelessWidget {
  final Datum? data;

  const PageDetailKorupsi(this.data, {super.key});

  Future<bool> _deleteKorupsi(String idKorupsi) async {
    final response = await http.post(
      Uri.parse('http://192.168.100.6/informasiDb/deleteKorupsi.php'),
      body: {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ElevatedButton(
              onPressed: () {
                _deleteKorupsi(data?.idKorupsi ?? '').then((isSuccess) {
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) {
                            return UpdateKorupsi(
                              idKorupsi: data?.idKorupsi ?? '',
                              idUser: data?.idUser ?? '',
