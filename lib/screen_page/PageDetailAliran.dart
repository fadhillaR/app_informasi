...
import 'package:app_informasi/screen_page/PageUpdateAliran.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:url_launcher/url_launcher.dart';

class PageDetailAliran extends StatelessWidget {
  final Datum? data;

  const PageDetailAliran(this.data, {super.key});

  Future<bool> _deleteAliran(String idAliran) async {
    final response = await http.post(
      Uri.parse('http://192.168.100.6/informasiDb/deleteAliran.php'),
      body: {
        "id_aliran": idAliran,
      },
    );
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ElevatedButton(
              onPressed: () {
                _deleteAliran(data?.idAliran ?? '').then((isSuccess) {
