...
class DetailAliranAdmin extends StatefulWidget {
  final Datum? data;

  const DetailAliranAdmin(this.data, {Key? key}) : super(key: key);

  @override
  _DetailAliranAdminState createState() => _DetailAliranAdminState();
}

class _DetailAliranAdminState extends State<DetailAliranAdmin> {
  Future<bool> _updateStatusLaporan(
    ...
    try {
      final response = await http.post(
        Uri.parse('http://192.168.100.6/informasiDb/updateStatusAliran.php'),
        body: {
          "id_aliran": idAliran,
                    ...
                    _updateStatusLaporan(
                      ...
                      if (isSuccess) {
                        ...
                        Navigator.pushAndRemoveUntil(
                            context,
                            MaterialPageRoute(
                                builder: (context) => ListAliranAdmin()),
                            (route) => false);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
