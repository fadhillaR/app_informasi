...
class DetailKorupsiAdmin extends StatefulWidget {
  final Datum? data;

  const DetailKorupsiAdmin(this.data, {Key? key}) : super(key: key);

  @override
  _DetailKorupsiAdminState createState() => _DetailKorupsiAdminState();
}

class _DetailKorupsiAdminState extends State<DetailKorupsiAdmin> {
  Future<bool> _updateStatusLaporan(
    ...
    try {
      final response = await http.post(
        Uri.parse('http://192.168.100.6/informasiDb/updateStatusKorupsi.php'),
        body: {
                    ...
                    _updateStatusLaporan(
                      ...
                      if (isSuccess) {
                        ...
                        Navigator.pushAndRemoveUntil(
                            context,
                            MaterialPageRoute(
                                builder: (context) => ListKorupsiAdmin()),
                            (route) => false);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
