...
class DetailHukumAdmin extends StatefulWidget {
  final Datum? data;

  const DetailHukumAdmin(this.data, {Key? key}) : super(key: key);

  @override
  _DetailHukumAdminState createState() => _DetailHukumAdminState();
}

class _DetailHukumAdminState extends State<DetailHukumAdmin> {
  Future<bool> _updateStatusLaporan(
    ...
    try {
      final response = await http.post(
        Uri.parse('http://192.168.100.6/informasiDb/updateStatusHukum.php'),
        body: {
          "id_hukum": idHukum,
                    ...
                    _updateStatusLaporan(
                      ...
                      if (isSuccess) {
                        ...
                        Navigator.pushAndRemoveUntil(
                            context,
                            MaterialPageRoute(
                                builder: (context) => ListHukumAdmin()),
                            (route) => false);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
