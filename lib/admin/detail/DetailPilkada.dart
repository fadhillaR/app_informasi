...
class DetailPilkadaAdmin extends StatefulWidget {
  final Datum? data;

  const DetailPilkadaAdmin(this.data, {Key? key}) : super(key: key);

  @override
  _DetailPilkadaAdminState createState() => _DetailPilkadaAdminState();
}

class _DetailPilkadaAdminState extends State<DetailPilkadaAdmin> {
  Future<bool> _updateStatusLaporan(
    ...
    try {
      final response = await http.post(
        Uri.parse('http://192.168.100.6/informasiDb/updateStatusPilkada.php'),
        body: {
                    ...
                    _updateStatusLaporan(
                      ...
                      if (isSuccess) {
                        ...
                        Navigator.pushAndRemoveUntil(
                            context,
                            MaterialPageRoute(
                                builder: (context) => ListPilkadaAdmin()),
                            (route) => false);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
