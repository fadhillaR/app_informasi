...
if(isset($_FILES['laporan'])) {
    ...
    if(move_uploaded_file($file_tmp, $file_destination)) {
        $update = "UPDATE pilkada SET id_user = '$id_user', isi_laporan = '$isi_laporan', laporan = '$file_name', status_laporan = '$status_laporan' WHERE id_pilkada = '$id_pilkada'";
        if (mysqli_query($koneksi, $update)) {
            $response['value'] = 1;
            $response['message'] = "Berhasil update data Posko Pilkada";
        } else {
            $response['value'] = 0;
            $response['message'] = "Gagal update data Posko Pilkada";
        }
    } else {
        $response['value'] = 0;
...
if(isset($_FILES['file_ktp'])) {
    ...
    if(move_uploaded_file($file_ktp_tmp, $file_ktp_destination)) {
        $update = "UPDATE pilkada SET id_user = '$id_user', isi_laporan = '$isi_laporan', file_ktp = '$file_ktp_name', status_laporan = '$status_laporan' WHERE id_pilkada = '$id_pilkada'";
        if (mysqli_query($koneksi, $update)) {
            $response['value'] = 1;
            $response['message'] = "Berhasil update data Posko Pilkada";
        } else {
...
if (!isset($_FILES['laporan']) && !isset($_FILES['file_ktp'])) {
    $update = "UPDATE pilkada SET id_user = '$id_user', isi_laporan = '$isi_laporan', status_laporan = '$status_laporan' WHERE id_pilkada = '$id_pilkada'";
    if (mysqli_query($koneksi, $update)) {
        $response['value'] = 1;
        $response['message'] = "Berhasil update data Posko Pilkada";
    } else {
