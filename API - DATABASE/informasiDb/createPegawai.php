...
if(isset($_FILES['file_ktp']) && isset($_FILES['laporan'])) {
    $file_name = basename($_FILES['laporan']['name']); // Mengambil nama file tanpa path
    $file_ktp_name = basename($_FILES['file_ktp']['name']);
    $file_tmp = $_FILES['laporan']['tmp_name'];
    $file_ktp_tmp = $_FILES['file_ktp']['tmp_name'];

    $file_destination = 'C:\xampp\htdocs\informasiDb\file\\' . $file_name; // lokasi penyimpanan file di server
    ...
    if(move_uploaded_file($file_ktp_tmp, $file_ktp_destination)) {
        ...
        if(move_uploaded_file($file_tmp, $file_destination)) {
            ...
            $insert = "INSERT INTO pegawai (id_user, file_ktp, isi_laporan, laporan, status_laporan) VALUES ('$id_user', '$file_ktp_name', '$isi_laporan', '$file_name', '$status_laporan')";
            
            if (mysqli_query($koneksi, $insert)) {
                $response['value'] = 1;
                $response['message'] = "Berhasil tambah data pengaduan pegawai";
            } else {
                $response['value'] = 0;
                $response['message'] = "Gagal tambah data pengaduan pegawai";
            }
        } else {
            $response['value'] = 0;
