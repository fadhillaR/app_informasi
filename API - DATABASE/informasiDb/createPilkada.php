...
if(isset($_FILES['file_ktp']) && isset($_FILES['laporan'])) {
    ...
    if(move_uploaded_file($file_ktp_tmp, $file_ktp_destination)) {
        ...
        if(move_uploaded_file($file_tmp, $file_destination)) {
            ...
            $insert = "INSERT INTO pilkada (id_user, file_ktp, isi_laporan, laporan, status_laporan) VALUES ('$id_user', '$file_ktp_name', '$isi_laporan', '$file_name', '$status_laporan')";
            
            if (mysqli_query($koneksi, $insert)) {
                $response['value'] = 1;
                $response['message'] = "Berhasil tambah data Posko Pilkada";
            } else {
                $response['value'] = 0;
                $response['message'] = "Gagal tambah data Posko Pilkada";
            }
        } else {
            $response['value'] = 0;
